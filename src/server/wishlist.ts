...
import {
  isValidHttpUrl,
  joinWishlistProductUrls,
  parseWishlistProductUrls,
} from "@/lib/utils/wishlist-links";
...
export const upsertWishlistItemFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: formData }) => {
    ...
    const productUrlsRaw = normalizeTextField(
      formData.get("product_urls") ?? formData.get("product_url"),
    );
