...
export async function SpecialDaysPageContent() {
  const [profile, specialDays] = await Promise.all([getCoupleProfile(), getSpecialDays()]);
    ...
    <>
      <div className="mt-8 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="card p-6">
          <p className="text-sm text-mocha/70 dark:text-white/55">Ngày bắt đầu hành trình của chúng mình</p>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-2xl bg-blush p-4 dark:bg-white/5">
              ...
              <p className="mt-1 text-2xl font-semibold dark:text-white">{specialDays.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="card p-6">
          ...
          {nextMilestone ? (
            <>
              <h2 className="mt-2 text-2xl font-semibold dark:text-white">{nextMilestone.title}</h2>
...
export function SpecialDaysContentFallback() {
