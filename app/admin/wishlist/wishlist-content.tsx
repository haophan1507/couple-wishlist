...
import { getCoupleProfile, getWishlistItemsPaginated } from "@/lib/data/queries";
import { parseWishlistProductUrls } from "@/lib/utils/wishlist-links";

export async function AdminWishlistPageContent({
  searchParams
}: {
  ...
  const [{ items, total }, profile] = await Promise.all([
    getWishlistItemsPaginated(page, pageSize),
    getCoupleProfile(),
  ]);
