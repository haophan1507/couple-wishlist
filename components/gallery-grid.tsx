...
export function GalleryGrid({ items }: { items: GalleryItem[] }) {
  if (!items.length) {
    return <p className="card p-8 text-center text-mocha/70">Chưa có kỷ niệm nào. Hãy thêm ảnh đầu tiên trong trang quản trị.</p>;
  }
      ...
      {items.map((item) => (
        <article key={item.id} className="card mb-4 break-inside-avoid p-3">
          ...
          <div className="px-1 pb-1 pt-3">
            ...
            {item.memory_date ? (
              <p className="mt-1 text-xs text-mocha/65">{format(new Date(item.memory_date), "PPP")}</p>
            ) : null}
          </div>
        </article>
      ))}
