...
export function ReserveGiftForm({ itemId }: { itemId: string }) {
  ...
  return (
    <form action={action} className="mt-4 space-y-3 border-t border-mocha/10 pt-4">
      ...
      <SubmitButton />
      {state.message ? (
        <p className={`text-xs ${state.success ? "text-green-700" : "text-red-600"}`}>{state.message}</p>
      ) : null}
    </form>
  );
}
